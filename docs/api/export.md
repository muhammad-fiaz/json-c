...
- `allocator`: optional custom allocator for the returned buffer
...
Serialization functions that return text allocate the result through the caller-selected allocator. The caller releases the buffer with jsonc_free_string or the matching allocator callback.
